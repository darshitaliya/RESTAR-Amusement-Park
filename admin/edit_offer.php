...
// Construct the query
$q = "SELECT * FROM offer WHERE id=$eid";
...
if (isset ($_REQUEST['add_offer'])) {
    ...
    $enm = $_REQUEST['offer_name'];
    $info = $_REQUEST['max_person'];
    $sdt = $_REQUEST['offer_price'];
    ...
    $q = "update offer set toffer='$enm',Person='$info',Price='$sdt',offerprice='$oprice',start_date='$sdate',end_date='$edate',image='$img_name' where id=$eid";
    echo $q;
    if (mysqli_query($conn, $q)) {
        echo "<script type='text/javascript'>window.location.href='./offers.php'</script>";
        // header("Location: ../Event.php",true);
        // echo "Updated Successfully!!";
        move_uploaded_file($tmp_loc, 'images/offers/' . $img_name);
  ...
  <form method="post" action="" enctype="multipart/form-data">
    <div class="row">
      ...
      <div class="col-md-12">
        ...
        <input type="text" class="form-control" id="oprice" name="oprice" required
        value="<?= $r[4]; ?>">
        </div>
      </div>
                        <div class="row">
            ...
            <div class="hero">
                <label for="input-file" id='drop-area'>
                    ...
                    <div id="img-view" class=''
                        style="background-image:url('<?php echo "images/offers/$r[7]"; ?>')"></div>
                </label>
    </div>
            </div>
    </div>
    <div class="row">
      <div class="col-md-12">
        <button type="submit" class="btn btn-primary" name="add_offer">Edit Offer</button>
      </div>
    </div>
  </form>
