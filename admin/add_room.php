...
if (isset ($_POST['add_room'])) {
  ...
  if (file_exists("./admin/images/room/" . $_FILES["room_image"]["name"])) {
    $store = $_FILES["room_image"]["name"];
    $_SESSION['status'] = "Image already exists: '$store'";
    header('Location:add_room.php');
  } else {
    // Insert into the database
    $query = "INSERT INTO rooms(`room_type`,`max_person`,`size`,`bed`,`price`,`description`,`image`) 
              VALUES('$room_type','$max_person','$size','$bed','$price','$description','$room_image')";

    $query_run = mysqli_query($conn, $query);

    if ($query_run) {
      move_uploaded_file($_FILES["room_image"]['tmp_name'], "./admin/images/room/" . $_FILES["room_image"]["name"]);
...
<body>
  ...
  <script>
    ...
    let room1 = document.forms["room"]["room_name"].value;
    let room2 = document.forms["room"]["room_price"].value;
    let room3 = document.forms["room"]["description"].value;
    let room4 = document.forms["room"]["max_person"].value;
    let room5 = document.forms["room"]["size"].value;
    let room6 = document.forms["room"]["bed"].value;

    if (room1 == "") {
        document.getElementById("room1").innerHTML = "Room type is required";
        valid = false;
    }
    if (room2 == "") {
        document.getElementById("room2").innerHTML = "Price is required";
        valid = false;
    }
    if (room3 == "") {
        document.getElementById("room3").innerHTML = "Description is required";
        valid = false;
    }
    if (room4 == "") {
        document.getElementById("room4").innerHTML = "Max person is required";
        valid = false;
    }
    if (room5 == "") {
        document.getElementById("room5").innerHTML = "Size is required";
        valid = false;
    }
    if (room6 == "") {
        document.getElementById("room6").innerHTML = "Bed is required";
        valid = false;
    }
